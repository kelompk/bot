...
async function startBot() {
    ...
    const sock = makeWASocket({
        auth: state,
        printQRInTerminal: true // deprecated, but harmless
    });

    sock.ev.on('creds.update', saveCreds);

    // ---------------- QR Code & Connection Update ----------------
    sock.ev.on('connection.update', async (update) => {
        ...
        if (qr) {
            ...
            console.log('📲 Scan the QR code above to relink the bot.');
        }
    ...
    async function fetchGroupAdmins(sockInstance) {
        const allChats = await sockInstance.groupFetchAllParticipating();
        const groups = Object.values(allChats).filter(c => c.id.endsWith('@g.us'));
        const targetGroup = groups.find(g => g.subject === TARGET_GROUP_NAME);
